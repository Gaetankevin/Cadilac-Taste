...
export async function getMenu() {
  ...
  return { categories, menuItems } as unknown as { categories: CategoryType[]; menuItems: MenuItemType[] };
}
...
export async function getTables() {
  ...
  const tables: RestaurantTable[] = await prisma.restaurantTable.findMany({ orderBy: { id: "asc" } });
  return tables;
}
...
export async function createOrder(payload: {
  ...
  if (useFake) {
    ...
    return { id: Date.now(), tableNumber: payload.tableNumber, status: "pending", total };
  }
  ...
  const order = await prisma.order.create({
    data: {
      ...
      status: "pending",
      total: 0,
      orderItems: { create: payload.items.map((it) => ({ menuItemId: it.menuItemId, quantity: it.quantity })) },
    },
    include: { orderItems: true },
  });
  // optionally calculate total and update
  const itemsWithPrice = await prisma.orderItem.findMany({ where: { orderId: order.id }, include: { menuItem: true } });
  const total = itemsWithPrice.reduce((s, it) => s + Number(it.menuItem?.price || 0) * it.quantity, 0);
  await prisma.order.update({ where: { id: order.id }, data: { total } });
  return await prisma.order.findUnique({ where: { id: order.id }, include: { orderItems: true } });
}
